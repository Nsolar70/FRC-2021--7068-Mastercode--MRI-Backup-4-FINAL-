...
import frc.robot.commands.RunShooterDown;
...
public class RobotContainer {
  ...
  private final StopperMotor m_stopperMotor = new StopperMotor();
 ...
 private final CommandBase m_manualyShoot = new ManualyShoot(m_flyWheel, m_indexer,m_stopperMotor);
 private final CommandBase m_manualyIndexDown = new ManualyIndexDown(m_indexer);
 private final CommandBase m_autoShoot = new AutoShoot(m_driveTrain, m_flyWheel, m_indexer, m_turret, m_stopperMotor);
 private final CommandBase m_runShooterDown = new RunShooterDown(m_stopperMotor, m_flyWheel);
  ...
  public RobotContainer() {
    ...
    m_driveTrain.setDefaultCommand(new RunCommand(
        () -> m_driveTrain.drive(m_driverController.getY(GenericHID.Hand.kLeft),
            m_driverController.getX(GenericHID.Hand.kLeft), m_driverController.getX(GenericHID.Hand.kRight)),
        m_driveTrain));
...
// Run the flywheel and stopper motor down when button is pressed
//new JoystickButton(m_actuatorController, Button.kA.value).whileHeld(m_runShooterDown);
